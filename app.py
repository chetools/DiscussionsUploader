...
from content import extract_from_file
from pdf_equations import Equation, parse_equation_range, renumber_label
...
def on_file_upload(upload_file) -> tuple[pd.DataFrame, str]:
    global _equations_cache, _source_kind
    if upload_file is None:
        _equations_cache = []
        _source_kind = ""
        return pd.DataFrame(), "Upload a PDF or Markdown file to extract equations."

    try:
        result = extract_from_file(upload_file)
        _equations_cache = result.equations
        _source_kind = result.source_kind
    except Exception as exc:
        _equations_cache = []
        _source_kind = ""
        return pd.DataFrame(), f"Could not read that file: {exc}"

    if not _equations_cache:
        label_hint = (
            "Labels should use **\\\\tag{1}**, **\\\\tag{3b}**, or **\\\\tag{5-1}** in Markdown."
            if _source_kind == "markdown"
            else "Labels should look like **(1)**, **(3b)**, or **(5-1)** in the PDF."
        )
        return pd.DataFrame(), f"No numbered equations found. {label_hint}"

    page_col = "Section" if _source_kind == "markdown" else "Page"
    df = pd.DataFrame(
        {
            "Equation #": [eq.label for eq in _equations_cache],
            page_col: [eq.page for eq in _equations_cache],
            "Equation": [eq.latex for eq in _equations_cache],
        }
    )
    kind_label = "Markdown" if _source_kind == "markdown" else "PDF"
    return (
        df,
        f"Found **{len(_equations_cache)}** equations from {kind_label}. "
        "Review them below, then continue to Step 3.",
    )
...
def build_preview(
    ...
    if not _equations_cache:
        return pd.DataFrame(), "Upload a PDF or Markdown file in Step 2 first."
...
def build_app() -> gr.Blocks:
    with gr.Blocks(title="Canvas Discussion: Equations") as app:
        gr.Markdown(
            ...
            f"Extract numbered equations from a PDF or Markdown file and save them as an **unpublished** Canvas discussion "
        ...
        with gr.Group(elem_classes=["section-card"]):
            gr.Markdown("## Step 2 — Upload file", elem_classes=["step-heading"])
            gr.Markdown(
                "Upload a **PDF** with labels like **(1)**, **(3b)**, or **(5-1)**, "
                "or a **Markdown** file with numbered display equations using **\\\\tag{1}**, **\\\\tag{3b}**, etc. "
                "Only numbered equations are uploaded; prose is stripped. "
                "The first PDF scan may take up to a minute while the equation reader loads."
            )
            pdf_in = gr.File(label="PDF or Markdown file", file_types=[".pdf", ".md"])
            extract_status = gr.Markdown("Upload a PDF or Markdown file to extract equations.")
        ...
        pdf_in.change(on_file_upload, inputs=pdf_in, outputs=[extracted_df, extract_status])
        preview_btn.click(
            build_preview,
